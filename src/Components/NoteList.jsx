...
const NotesList = ({ setisConect, isConnect }) => {
    ...
    axios
      ...
      .then((response) => {
        const filteredNotes = response.data.filter(
          (note) =>
            note.shared_with !== undefined && note.shared_with.length > 0
        );
  ...
  return (
    <div className="test">
      <Navbar showCreateNote={showCreateNote} setisConect={setisConect} />
      <div
        className="testNote"
